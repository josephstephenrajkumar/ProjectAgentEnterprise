...
2. 'contract_sow_agent': Document-based questions about SOW, contracts, clauses, obligations, deliverables, scope, work packages, milestones, acceptance criteria, pricing, commercial clauses.
...
8. 'data_quality_agent': Detect missing actuals, stale forecasts, mismatched hours.
9. 'mbr_summary_agent': Draft MBR or weekly summary commentary.
10. 'delete_agent': Delete or remove a project.
11. 'email_agent': Send an email or forward information.
12. 'both': The query explicitly needs BOTH SQL and RAG agents combined.
13. 'general_agent': General conversation, off-topic, or greetings.
...
def _llm_route(query: str, history: list[dict] | None = None) -> str:
    ...
    valid_keys = [
        "sql_agent", "forecast_variance_agent", "metrics_agent",
...
def router_node(state: AgentState) -> dict:
    ...
    if not decision:
        ...
        routing_type = "LLM fallback"

    # Intercept sql_agent to prevent infinite loop since SQL Agent already ran as entry point
    if decision == "sql_agent":
        debug += f"\n🔄 Router: Intercepted sql_agent route to prevent infinite loop. Rerouting to contract_sow_agent as document fallback."
        decision = "contract_sow_agent"

    return {
        "next_node": decision,
        "debug_log": debug + f"\n🚦 Router → {decision} ({routing_type})",
        "agent_outputs": [],
        "last_route": decision,
    }
